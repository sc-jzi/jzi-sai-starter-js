...
/* PentairFlow variant — image-first tiles, green rectangular CTA */
export const PentairFlow = (props: TwoColumnCtaProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  ...
  const Tile = ({
    ...
    const hasLink = Boolean(link?.value?.href && link.value.href !== '#');
    return (
      <div className="col-sm-12 col-lg-6">
        <div className="content-wrapper">
          <NextImage field={image} width={800} height={480} />
          {(isPageEditing || title?.value) && (
            <h2>
              <Text field={title} />
            </h2>
  ...
  return (
    <div className={`component two-column-cta pentair-flow-tiles ${sxaStyles}`} id={id ? id : undefined}>
      <div className="container">
        <div className="row">
          <Tile
            image={props.fields.Image1}
