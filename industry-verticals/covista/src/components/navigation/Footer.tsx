...
export const Covista = (props: FooterProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const sxaStyles = `${props.params?.styles || ''}`;

  return (
    <div className={`component footer cv-footer ${sxaStyles}`} id={id ? id : undefined}>
      <div className="container">
        <div className="cv-footer__top">
          <div className="cv-footer__brand">
            <NextImage field={props.fields?.Image1} width={180} height={40} />
            <p className="cv-footer__address">
              <Text field={props.fields?.Copyright} />
            </p>
          </div>
          <div className="cv-footer__cols">
            <div>
              <div className="cv-footer__title">
                <Text field={props.fields?.Title1} />
              </div>
