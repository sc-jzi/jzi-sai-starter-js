...
const AkamaiWhatsNewDefault = (props: ArticleListComponentProps): JSX.Element => {
  const id = props.params?.RenderingIdentifier;
  const sxaStyles = `${props.params?.styles || ''}`;
  const scrollerRef = useRef<HTMLDivElement>(null);
  const numOfItems = parseInt(props.params?.NumberOfItems, 10) || 5;
...
export const WhatsNew = withDatasourceCheck()<ArticleListComponentProps>(AkamaiWhatsNewDefault);