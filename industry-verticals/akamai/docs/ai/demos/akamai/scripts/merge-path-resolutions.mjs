/**
 * Merge _five-product-paths.json + _path-resolutions.jsonl -> product-ds-created-five-products.json
 * jsonl lines: { product, key, name, itemPath, itemId?, error? }
 */
...
for (const row of paths) {
  ...
  if (hit?.itemId) {
    ...
    failures.push({ ...row, reason: hit?.error || 'missing resolution' });
  }
}
