...
/* Pentair variant — full-bleed overlay, white CTA, triangle flourish */
export const Pentair = (props: AppPromoProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  ...
  return (
    <div className={`component hero pentair ${sxaStyles}`} id={id ? id : undefined}>
      <picture>
