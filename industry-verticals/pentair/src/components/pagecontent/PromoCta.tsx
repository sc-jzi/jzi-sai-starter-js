...
/* Pentair variant — text left / image right, no dotted accents, green text link */
export const Pentair = (props: PromoCtaProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  const mutedClass = props.fields?.Eyebrow?.value ? 'pentair-muted' : '';

  return (
    <div
      className={`component promo-cta pentair ${mutedClass} ${sxaStyles}`}
      id={id ? id : undefined}
    >
      <div className="container">
        <div className="row row-gap-4 main-content align-items-center">
...
/* PentairAlt variant — image left / text right */
export const PentairAlt = (props: PromoCtaProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  ...
  return (
    <div className={`component promo-cta pentair pentair-alt ${sxaStyles}`} id={id ? id : undefined}>
      <div className="container">
        <div className="row row-gap-4 main-content align-items-center">
          <div className="col-md-10 mx-auto col-lg-7 mx-lg-0 order-lg-1">
            <div className="image-wrapper">
              <NextImage
                field={props.fields.Image}
                className="d-block mx-lg-auto img-fluid"
          ...
          <div className="col-lg-5 text-center text-lg-start order-lg-2">
            {(isPageEditing || props.fields?.Eyebrow?.value) && (
              <h6 className="eyebrow-accent">
                <Text field={props.fields.Eyebrow} />
              </h6>
